...
# CONTRACTILES ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcontractiles.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── contractiles.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── contractiles.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/contractiles.h
```
...
```c
#include "contractiles.h"

int main() {
    void* handle = contractiles_init();
    if (!handle) return 1;

    int result = contractiles_process(handle, 42);
    if (result != 0) {
        const char* err = contractiles_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    contractiles_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lcontractiles -L./zig-out/lib
```

### From Idris2

```idris
import CONTRACTILES.ABI.Foreign
...
```rust
#[link(name = "contractiles")]
extern "C" {
    fn contractiles_init() -> *mut std::ffi::c_void;
    fn contractiles_free(handle: *mut std::ffi::c_void);
    fn contractiles_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = contractiles_init();
        assert!(!handle.is_null());

        let result = contractiles_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:contractiles_init, libcontractiles), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:contractiles_process, libcontractiles), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:contractiles_free, libcontractiles), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/contractiles.h
   ```
